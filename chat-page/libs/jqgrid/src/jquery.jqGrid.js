			...
			var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
				...
				emptyRows = function(scroll, locdata) {
					...
					if (scroll && this.p.scroll) {
						...
						//$(this.grid.bDiv.firstChild).css({height: "0"});////////////////////////////////////////////////////////
						$(this.grid.bDiv.firstChild.firstChild).css({
							height: "0px",
							display: "none"
						});
						//$(this.grid.bDiv.firstChild.firstChild).css({height: "0px"}).hide();
				...
				addXmlData = function(xml, rcnt, more, adjust) {
					...
					var gl = gxml.length,
						...
						altr, tablebody = $("#" + $.jgrid.jqID(ts.p.id) + " tbody:first");
					if (gl > 0 && ts.p.page <= 0) {
						ts.p.page = 1;
					}
					if (gxml && gl) {
						...
						var afterInsRow = $.isFunction(ts.p.afterInsertRow),
							hiderow = false,
							groupingPrepare, rnc = ni ? getstyle(stylemodule, 'rownumBox', false, 'jqgrid-rownum') : "",
				...
				addJSONData = function(data, rcnt, more, adjust) {
					...
					var afterInsRow = $.isFunction(ts.p.afterInsertRow),
						grpdata = [],
						hiderow = false,
						groupingPrepare, tablebody = $("#" + $.jgrid.jqID(ts.p.id) + " tbody:first"),
				...
				updatepager = function(rn, dnd) {
					var cp, last, base, from, to, tot, fmt, pgboxes = "",
						sppg, pgid = ts.p.pager ? $.jgrid.jqID(ts.p.pager.substr(1)) : "",
						tspg = pgid ? "_" + pgid : "",
				...
				setPager = function(pgid, tp) {
					...
					var sep = "<td class='ui-pg-button " + disabled + "'><span class='ui-separator'></span></td>",
						...
						str = "",
						pgcnt, lft, cent, rgt, twd, tdw, i, clearVals = function(onpaging, thus) {
							var ret;
					...
					cent = pgid + "_center";
					rgt = pgid + "_right";
					$("#" + $.jgrid.jqID(pgid)).append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><table " + getstyle(stylemodule, 'pagerTable', false, 'ui-pg-table ui-common-table ui-pager-table') + "><tbody><tr><td id='" + lft + "' align='left'></td><td id='" + cent + "' align='center' style='white-space:pre;'></td><td id='" + rgt + "' align='right'></td></tr></tbody></table></div>").attr("dir", "ltr"); //explicit setting
					...
					if (ts.p.pgbuttons === true) {
						var po = ["first" + tp, "prev" + tp, "next" + tp, "last" + tp],
							...
							pot = [($.jgrid.getRegional(ts, "defaults.pgfirst", ts.p.pgfirst) || ""), ($.jgrid.getRegional(ts, "defaults.pgprev", ts.p.pgprev) || ""), ($.jgrid.getRegional(ts, "defaults.pgnext", ts.p.pgnext) || ""), ($.jgrid.getRegional(ts, "defaults.pglast", ts.p.pglast) || "")];
						if (dir === "rtl") {
							po.reverse();
							pot.reverse();
						}
...
/*
			...
			*/
				...
				buildColItems = function(top, left, parent) {
					var cm = ts.p.colModel,
						len = cm.length,
						i, cols = [],
						disp, texts = $.jgrid.getRegional(ts, "colmenu"),
				...
				buildSearchBox = function(index, top, left, parent) {
					var cm = ts.p.colModel[index],
						...
						v2 = '',
						so, op, repstr = '',
						selected, elem, numopts = ['eq', 'ne', 'lt', 'le', 'gt', 'ge', 'nu', 'nn', 'in', 'ni'],
				...
				buildColMenu = function(index, left, top) {
					...
					function() {
						...
						if ($(this).attr("value") === 'columns') {
							left1 = $(this).parent().width() + 18, top1 = $(this).parent().position().top - 5;
			...
			$("th", thr).each(function(j) {
				...
				//firstr += "<td role='gridcell' style='display:none;height:0px;width:"+w+"px;"+hdcol+"'></td>";
				grid.headers[j] = {
					width: w,
					el: this
				};
			...
			$(grid.cDiv).append(arf).append("<span class='ui-jqgrid-title'>" + ts.p.caption + "</span>").addClass("ui-jqgrid-titlebar ui-jqgrid-caption" + (dir === "rtl" ? "-rtl" : "") + " " + getstyle(stylemodule, 'gridtitleBox', true));
	...
	$.jgrid.extend({
		...
		setSelection: function(selection, onsr, e) {
			return this.each(function() {
				var $t = this,
					stat, pt, ner, ia, tpsr, fid, csr, getstyle = $.jgrid.getMethod("getStyleUI"),
		...
		resetSelection: function(rowid) {
			return this.each(function() {
				var t = this,
					sr, fid, getstyle = $.jgrid.getMethod("getStyleUI"),
					highlight = getstyle(t.p.styleUI + '.common', 'highlight', true),
			...
			if (rdata) {
				...
				this.each(function() {
					...
					var k = 0,
						...
						lcdata = {},
						classes, air = $.isFunction(t.p.afterInsertRow) ? true : false;
					if (ni) {
		...
		progressBar: function(p) {
			...
			return this.each(function() {
				var sh = p.method === "show" ? true : false,
					...
					offsetParent, top, scrollTop = $(window).scrollTop();
				if (p.htmlcontent !== "") {
				...
				if (loadDiv.is(':visible')) {
					...
					if (loadDiv.offset().top < scrollTop) {
						top = Math.min(
						10 + scrollTop - offsetParent.offset().top, offsetParent.height() - loadDiv.height());
				...
				if (maxfrozen >= 0 && frozen) {
					...
					$($t).on('jqGridAfterGridComplete.setFrozenColumns', function() {
						...
						$("#" + $.jgrid.jqID($t.p.id) + " tr[role=row].jqgrow").each(function() {
							mh.push($("td:visible:first", this).height());//////////////////////////////////rox-----------------------
						});
						
						var btbl = $("#" + $.jgrid.jqID($t.p.id)).clone(true);
						$(btbl).height($("#" + $.jgrid.jqID($t.p.id)).height());
						$("tr[role=row]", btbl).each(function() {
							if ($("td:first").height() == 0) $(this).height(0);	//////////////////////////////////rox-----------------------
	...
	$.jgrid.extend({
		editCell: function(iRow, iCol, ed) {
			return this.each(function() {
				var $t = this,
					nm, tmp, cc, cm, highlight = $(this).jqGrid('getStyleUI', $t.p.styleUI + '.common', 'highlight', true),
		...
		saveCell: function(iRow, iCol) {
			return this.each(function() {
				var $t = this,
					fr, errors = $.jgrid.getRegional(this, 'errors'),
	...
	$.extend($.jgrid, {
		...
		createModal: function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
			...
			var ahr = $("<a class='ui-jqdialog-titlebar-close " + common.cornerall + "'></a>").hover(function() {
		...
		info_dialog: function(caption, content, c_b, modalopt) {
			...
			cnt += c_b ? "<div class='" + classes.content + "' style='text-align:" + mopt.buttonalign + ";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a id='closedialog' class='fm-button " + common.button + "'>" + c_b + "</a>" + buttstr + "</div>" : buttstr !== "" ? "<div class='" + classes.content + "' style='text-align:" + mopt.buttonalign + ";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'>" + buttstr + "</div>" : "";
			cnt += "</div>";
		...
		createEl: function(eltype, options, vl, autowidth, ajaxso) {
			...
			case "select":
				...
				if (options.dataUrl != null) {
					...
					$.ajax($.extend({
						...
						success: function(data) {
							var ovm = [],
								...
								oV = '',
								txt, a = $.isFunction(options.buildSelect) ? options.buildSelect.call($t, data) : data;
							if (typeof a === 'string') {
								a = $($.trim(a)).html();
							}
							if (a) {
					...
					var so, sv, ov, oSv, key, value, sep = options.separator === undefined ? ":" : options.separator,
		...
		*/
			...
			this.createTableForGroup = function(group, parentgroup) {
				...
				if (this.p.ruleButtons === true) {
					...
					var inputAddRule = $("<input type='button' value='+' title='" + that.p.addrule + "' class='add-rule ui-add " + common.button + "'/>"),
						cm;
			...
			this.createTableRowForRule = function(rule, group) {
				...
				$(ruleDataInput).addClass("input-elm " + classes.srInput).on('change', function() {
			...
			return this.each(function() {
				...
				var classes = $.jgrid.styleUI[($t.p.styleUI || 'jQueryUI')].filter,
					...
					triggerToolbar = function() {
						...
						$.each($t.p.colModel, function() {
							...
							v = this.stype === "custom" && $.isFunction(sop.custom_value) && $elem.length > 0 && $elem[0].nodeName.toUpperCase() === "SPAN" ? sop.custom_value.call($t, $elem.children(".customelement:first"), "get") : $elem.val();
							// detect multiselect
				...
				$.each($t.p.colModel, function(ci) {
					var cm = this,
						soptions, select = "",
						sot = "=",
						so, i, st, csv, df, elem, restores, th = $("<th role='columnheader' class='" + base.headerBox + " ui-th-" + $t.p.direction + "' id='gsh_" + $t.p.id + "_" + cm.name + "' ></th>"),
						thd = $("<div></div>"),
		...
		searchGrid: function(p) {
			...
			return this.each(function() {
				...
				var fid = "fbox_" + $t.p.id,
					...
					fl, classes = $.jgrid.styleUI[($t.p.styleUI || 'jQueryUI')].filter,
					common = $.jgrid.styleUI[($t.p.styleUI || 'jQueryUI')].common;
	...
	$.jgrid.extend({
		editGridRow: function(rowid, p) {
			...
			return this.each(function() {
				...
				var gID = $t.p.id,
					...
					maxCols = 1,
					maxRows = 0,
					postdata, diff, frmoper, templ = typeof rp_ge[$t.p.id].template === "string" && rp_ge[$t.p.id].template.length > 0,
				...
				if (p.checkOnUpdate) {
					...
					$("a.ui-jqdialog-titlebar-close", "#" + $.jgrid.jqID(IDs.themodal)).off("click").click(function() {
						if (!checkUpdates()) {
							return false;
						}
						$.jgrid.hideModal("#" + $.jgrid.jqID(IDs.themodal), {
				...
				if (p.saveicon[0] === true) {
					$("#sData", frmtb2).addClass(p.saveicon[1] === "right" ? 'fm-button-icon-right' : 'fm-button-icon-left').append("<span class='" + commonstyle.icon_base + " " + p.saveicon[2] + "'></span>");
				}
				if (p.closeicon[0] === true) {
					$("#cData", frmtb2).addClass(p.closeicon[1] === "right" ? 'fm-button-icon-right' : 'fm-button-icon-left').append("<span class='" + commonstyle.icon_base + " " + p.closeicon[2] + "'></span>");
				}
				if (rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
			...
			return this.each(function() {
				...
				function createData(rowid, obj, tb, maxcols) {
					var nm, hc, trdata, cnt = 0,
						tmp, dc, retpos = [],
						ind = false,
						i, tdtmpl = "<td class='CaptionTD form-view-label " + commonstyle.content + "' width='" + p.labelswidth + "'>&#160;</td><td class='DataTD form-view-data ui-helper-reset " + commonstyle.content + "'>&#160;</td>",
						tmpl = "",
				...
				p.closeicon = $.extend([true, "left", styles.icon_close], p.closeicon);
				if (p.closeicon[0] === true) {
					$("#cData", "#" + frmtb + "_2").addClass(p.closeicon[1] === "right" ? 'fm-button-icon-right' : 'fm-button-icon-left').append("<span class='" + commonstyle.icon_base + " " + p.closeicon[2] + "'></span>");
				}
		...
		delGridRow: function(rowids, p) {
			...
			return this.each(function() {
				...
				if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
					...
					if (p.delicon[0] === true) {
						$("#dData", "#" + dtbl + "_2").addClass(p.delicon[1] === "right" ? 'fm-button-icon-right' : 'fm-button-icon-left').append("<span class='" + commonstyle.icon_base + " " + p.delicon[2] + "'></span>");
					}
					if (p.cancelicon[0] === true) {
						$("#eData", "#" + dtbl + "_2").addClass(p.cancelicon[1] === "right" ? 'fm-button-icon-right' : 'fm-button-icon-left').append("<span class='" + commonstyle.icon_base + " " + p.cancelicon[2] + "'></span>");
					}
		...
		navGrid: function(elem, p, pEdit, pAdd, pDel, pSearch, pView) {
			...
			return this.each(function() {
				...
				for (i = 0; i < clone; i++) {
					var tbd, navtbl = $("<table class='ui-pg-table navtable ui-common-table'><tbody><tr></tr></tbody></table>"),
		...
		_buildNavMenu: function(elem, elemids, p, pEdit, pAdd, pDel, pSearch, pView) {
			return this.each(function() {
				...
				var alertIDs = {
					...
					_buildMenu = function() {
						var fs = $('.ui-jqgrid-view').css('font-size') || '11px',
							eid, itm, str = $('<ul id="' + mid + '" class="ui-nav-menu modal-content" role="menu" tabindex="0" style="display:none;font-size:' + fs + '"></ul>');
						if (p.add) {
							pAdd = pAdd || {};
		...
		setGroupHeaders: function(o) {
			...
			return this.each(function() {
				var ts = this,
					...
					//startColumnName,
					numberOfColumns, titleText, cVisibleColumns, className, colModel = ts.p.colModel,
					cml = colModel.length,
					ths = ts.grid.headers,
					...
					if (iCol >= 0) {
						...
						if (skip === 0) {
							if (o.useColSpanStyle) {
								...
								$('<th>', {
									role: "columnheader"
								}).addClass(base.headerBox + " ui-th-column-header ui-th-" + ts.p.direction).css({
									"display": cmi.hidden ? 'none' : ''
								}).insertBefore($th);
								$tr.append(th);
							}
	...
	$.extend($.jgrid, {
		saveState: function(jqGridId, o) {
			o = $.extend({
				...
				// object by example gzip, LZString
				compressionMethod: 'compressToUTF16',
				// string by example zip, compressToUTF16
				debug: false
			}, o || {});
			if (!jqGridId) {
				return;
		...
		loadState: function(jqGridId, gridstring, o) {
			o = $.extend({
				...
				// clears the jqGrid localStorage items aftre load
				beforeSetGrid: null,
				afterSetGrid: null,
				decompression: false,
			...
			if (ret && $.type(ret) === 'object') {
				...
				if (ret.treeGrid) {
					...
					while (i < len) {
						$(grid[0].rows[i].cells[expCol]).find("div.treeclick").on("click", function(e) {
						...
						if (ret.ExpandColClick === true) {
							$(grid[0].rows[i].cells[expCol]).find("span.cell-wrapper").css("cursor", "pointer").on("click", function(e) {
								var target = e.target || e.srcElement,
									ind2 = $.jgrid.stripPref(ret.idPrefix, $(target, grid[0].rows).closest("tr.jqgrow")[0].id),
									pos = grid[0].p._index[ind2];
								if (!grid[0].p.data[pos][isLeaf]) {
		...
		editRow: function(rowid, keys, oneditfunc, successfunc, url, extraparam, aftersavefunc, errorfunc, afterrestorefunc) {
			...
			return this.each(function() {
				var $t = this,
					...
					focus = null,
					svr = {},
					ind, cm, bfer, inpclass = $(this).jqGrid('getStyleUI', $t.p.styleUI + ".inlinedit", 'inputClass', true);
				if (!$t.grid) {
					return;
				}
	...
	$.jgrid.extend({
		...
		columnChooser: function(opts) {
			var self = this,
				...
				fixedCols = [],
				dopts, mopts, $dialogContent, multiselectData, listHeight, colModel = self.jqGrid("getGridParam", "colModel"),
...
/* Function to cleanup the dialog, and select. Also calls the
	...
	function _pivotfilter(fn, context) { /*jshint validthis: true */
		var i, value, result = [],
			length;
	...
	$.jgrid.extend({
		pivotSetup: function(data, options) {
			...
			this.each(function() {
				...
				function findGroup(item, index) { /*jshint validthis: true */
					var j = 0,
						ret = true,
						i;
					for (i in item) {
	...
	$.jgrid.extend({
		setSubGrid: function() {
			return this.each(function() {
				var $t = this,
					cm, i, classes = $.jgrid.styleUI[($t.p.styleUI || 'jQueryUI')].subgrid,
					suboptions = {
		...
		addSubGrid: function(pos, sind) {
			return this.each(function() {
				...
				var subGridXml = function(sjxml, sbid) {
						var tddiv, i, sgmap, dummy = $("<table class='" + base.rowTable + " ui-common-table'><tbody></tbody></table>"),
							trdiv = $("<tr></tr>");
				...
				var subGridJson = function(sjxml, sbid) {
						var tddiv, result, i, cur, sgmap, j, dummy = $("<table class='" + base.rowTable + " ui-common-table'><tbody></tbody></table>"),
							trdiv = $("<tr></tr>");
						for (i = 0; i < ts.p.subGridModel[0].name.length; i++) {
							tddiv = $("<th class='" + base.headerBox + " ui-th-subgrid ui-th-column ui-th-" + ts.p.direction + "'></th>");
							$(tddiv).html(ts.p.subGridModel[0].name[i]);
		...
		setTreeNode: function(i, len) {
			return this.each(function() {
				...
				var expCol = $t.p.expColInd,
					...
					lft, rgt, curLevel, ident, lftpos, twrap, ldat, lf, common = $.jgrid.styleUI[($t.p.styleUI || 'jQueryUI')].common,
					index = i;
				...
				while (i < len) {
					...
					$($t.rows[i].cells[expCol]).find("div.treeclick").on("click", function(e) {
						var target = e.target || e.srcElement,
					...
					if ($t.p.ExpandColClick === true) {
						$($t.rows[i].cells[expCol]).find("span.cell-wrapper").css("cursor", "pointer").on("click", function(e) {
							var target = e.target || e.srcElement,
								ind2 = $.jgrid.stripPref($t.p.idPrefix, $(target, $t.rows).closest("tr.jqgrow")[0].id),
								pos = $t.p._index[ind2];
							if (!$t.p.data[pos][isLeaf]) {
		...
		SortTree: function(sortname, newDir, st, datefmt) {
			return this.each(function() {
				...
				var i, len, rec, records = [],
					$t = this,
					query, roots, rt = $(this).jqGrid("getRootNodes", $t.p.search);
				// Sorting roots
			...
			return this.each(function() {
				...
				if (rc !== undefined) {
					...
					if ($t.p.treeGridModel === "nested") {
						...
						res = $.jgrid.from.call($t, $t.p.data).greater(left, myright, {
							stype: 'integer'
						}).select();
						if (res.length) {
							for (key in res) {
						...
						res = $.jgrid.from.call($t, $t.p.data).greater(right, myright, {
							stype: 'integer'
						}).select();
						if (res.length) {
							for (key in res) {
				...
				if (rc !== undefined) {
					...
					if (dr.length > 0) {
						for (var i = 0; i < dr.length; i++) {
							if (dr[i][rid] !== rowid) $($t).jqGrid("delRowData", dr[i][rid]);
						}
					}
					if ($t.p.treeGridModel === "nested") {
						// ToDo - update grid data
			...
			if (data) {
				...
				if ($t.p.treeGridModel === "nested") {
					...
					if (parentid !== null) {
						...
						res = $.jgrid.from.call($t, $t.p.data).greater(right, maxright, {
							stype: 'integer'
						}).select();
						if (res.length) {
							for (key in res) {
...
*/
	...
	$.fn.fmatter.actions = function(cellval, opts) {
		var op = {
			...
			rowid = opts.rowId,
			str = "",
			ocl, nav = $.jgrid.getRegional(this, 'nav'),
	...
	$.fn.html5sortable = function(options) {
		...
		return this.each(function() {
			...
			var placeholder = $('<' + (/^ul|ol$/i.test(this.tagName) ? 'li' : /^tbody$/i.test(this.tagName) ? 'tr' : 'div') + ' class="sortable-placeholder ' + options.placeholderClass + '">').html('&nbsp;');
	...
	$.extend($.jgrid, {
		...
		xmlToZip: function(zip, obj) {
			var $t = this,
				...
				$.parseXML($.jgrid.excelStrings['xl/worksheets/sheet1.xml'])).indexOf('xmlns:r') === -1,
				newDir, worksheet, i, ien, attr, attrs = [],
				str;
			...
			$.each(obj, function(name, val) {
				if ($.isPlainObject(val)) {
					...
					str = str.replace(/<row xmlns="" /g, '<row ').replace(/<cols xmlns="">/g, '<cols>').replace(/<mergeCells xmlns="" /g, '<mergeCells ');
	...
	$.jgrid.extend({
		exportToCsv: function(p) {
					...
					$.each(grp.groups, function(i, n) {
						...
						if (leaf) {
							var gg = grp.groups[i + 1],
								kk, ik, offset = 0,
								sgr = n.startRow,
								to, end = gg !== undefined ? gg.startRow : grp.groups[i].startRow + grp.groups[i].cnt;
							for (kk = sgr; kk < end; kk++) {
								if (!grdata[kk - offset]) {
									break;
								}
			...
			if (p.returnAsString) {
				...
				jqGridUtils.saveAs(ret, p.fileName, {
					type: p.mimetype
				});
			}
		},
		...
		exportToExcel: function(o) {
			...
			this.each(function() {
				...
				var currentRow, rowNode, addRow = function(row, header) {
						currentRow = rowPos + 1;
						...
						for (var key in data.header[0]) {
							...
							if (typeof v === 'number' || (
							v.match && $.trim(v).match(/^-?\d+(\.\d+)?$/) && !$.trim(v).match(/^0\d+/))) {
								cell = $.jgrid.makeNode(rels, 'c', {
									...
									children: [
									$.jgrid.makeNode(rels, 'v', {
										text: v
									})]
								});
							} else {
								// Replace non standard characters for text output
								var text = !v.replace ? v : $.jgrid.htmlEncode(v);
								cell = $.jgrid.makeNode(rels, 'c', {
									attr: {
										t: 'inlineStr',
										r: cellId
				...
				function groupToExcel(grdata) {
					...
					function buildSummaryTd(i, ik, grp, foffset) {
						var fdata = findGroupIdx(i, ik, grp),
							//cm = $t.p.colModel,
							vv, grlen = fdata.cnt,
							k, retarr = emptyData(data.header[0]);
						for (k = foffset; k < colspans; k++) {
							if (cm[k].hidden) {
								continue;
							}
				...
				try {
					...
					if (zip.generateAsync) {
						...
						zip.generateAsync(zipConfig).then(function(blob) {
							jqGridUtils.saveAs(blob, o.fileName, {
								type: o.mimetype
							});
						});
					} else {
						// JSZip 2.5
						jqGridUtils.saveAs(zip.generate(zipConfig), o.fileName, {
							type: o.mimetype
						});
					}
				} catch (e) {
				...
				try {
					...
					if (o.download === 'open') {
						...
						pdf.getBuffer(function(buffer) {
							jqGridUtils.saveAs(buffer, o.fileName, {
								type: o.mimetype
							});
						});
					}
