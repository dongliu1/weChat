...
(function($, undefined){
	...
	$.widget("ui.rangeSliderDraggable", $.ui.rangeSliderMouseTouch, {
		...
		_mouseDrag: function(event){
			if(typeof this.cache.click =="undefined") this._mouseStart(event);		//rox
...
(function($, undefined){
	...
	$.widget("ui.rangeSliderBar", $.ui.rangeSliderDraggable, {
		...
		_setWheelModeOption: function(value){		//rox add scale option
		...
		_bindMouseWheel: function(mode){		//////////////rox-----1
			if (mode === "zoom"){
		...
		_mouseWheelZoom: function(event, delta, deltaX, deltaY){
			...
			//console.log("_mouseWheelZoom:"+deltaY)
			if (!this.enabled){
				return false;
			}
...
(function($, undefined){
	...
	$.widget("ui.rangeSliderLabel", $.ui.rangeSliderMouseTouch, {
		...
		_create: function(){
			...
			var obj=this;
			this.element.parent().on("mouseover",function(){
				obj._updateShowOption("show");
			}).on("mouseout",function(){
				obj._updateShowOption("hide");
			});
...
(function ($, undefined) {
	...
	$.widget("ui.rangeSlider", {
		...
		_reactivateLabels: function(){
			if (this.options.valueLabels === "change"){
				this._leftLabel("option", "show", this.options.valueLabels);
				this._rightLabel("option", "show", this.options.valueLabels);
			}
		},
		...
		_scrollRightClick: function(e){
			...
			// 原始-- this._continueScrolling("scrollRight", 4 * this._stepRatio(), 1);
			
			//以下为修改
			var value_max=this._values.max*2-this._values.min;
			var bound_max=value_max+(this.options.bounds.max-this.options.bounds.min)*0.05 ;
			
			this._setBoundsOption("bounds",{min:this.options.bounds.min,max:Math.max(value_max,bound_max)})
			this.values(this._values.max,value_max);
			this._trigger("userValuesChanged");
		},
		...
		_scrollLeftClick: function(e){
			...
			//原始-- this._continueScrolling("scrollLeft", 4 * this._stepRatio(), 1);
			
			//以下为修改
			var value_min=this._values.min*2-this._values.max;
			var bound_min=value_min-(this.options.bounds.max-this.options.bounds.min)*0.05 ;
			
			this._setBoundsOption("bounds",{min:Math.min(value_min,bound_min),max:this.options.bounds.max})
			this.values(value_min,this._values.min);
			this._trigger("userValuesChanged");
		},
	...
	$.widget("ui.dateRangeSliderHandle", $.ui.rangeSliderHandle, {
		...
		_createSteps: function(){
			...
			var minDate = new Date(this.options.bounds.min.valueOf()),
				...
				i = 0,
				previous = new Date();
			//console.log(new Date()+" row-2570");
			this._steps = [];
			var nMinDate=new Date(minDate.valueOf());
			while (stepDate <= maxDate && (i === 1 || previous.valueOf() !== stepDate.valueOf())){
				previous = stepDate;
				this._steps.push(stepDate.valueOf());

				stepDate = this._addStep(nMinDate, i, this.options.step);
				i++;
			}
			//console.log(new Date()+" row-2571");
		...
		_addStep: function(result, factor, step){	
		...
		_addThing: function(date, thing, factor, base){
			...
			//console.log(new Date()+" "+thing +" "+date["set" + thing]);
			return date;
		},
...
(function ($, undefined) {
	"use strict";
	var _cmonths=["1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月"],
		_weekdays=["D7","D1","D2","D3","D4","D5","D6"];
	$.widget("ui.dateRangeSlider", $.ui.rangeSlider, {
		options: {
			bounds: {min: new Date(2013,0,1).valueOf(), max: new Date(2015,0,1).valueOf()},
			defaultValues: {min: new Date(2014,1,11).valueOf(), max: new Date(2015,1,11).valueOf()},
			scales: [],
		},
		base_scale:{"year":{										//rox: 5 base scales type 
				first: function(value){var first = new Date(value);return new Date(first.setMonth(0,1)); },
				next: function(value){var next = new Date(value);return new Date(next.setFullYear(value.getFullYear()+1));},
				label: function(value){return value.getFullYear();}
			},"longmonth":{
				first: function(value){return moment(value).startOf('month'); },
				next: function(value){return new moment(value).add(1,'months');},
				label: function(value){return moment(value).format("YYYY-MM");}
			},"shortmonth":{
				first: function(value){return moment(value).startOf('month'); },
				next: function(value){return new moment(value).add(1,'months');},
				label: function(value){return moment(value).format("MM");}
			},"longweek":{
				first: function(value){return moment(value).startOf('week'); },
				next: function(value){return new moment(value).add(1,'weeks');},
				label: function(value){return moment(value).format("YYYY-MM-DD");}
			},"shortweek":{
				first: function(value){return moment(value).startOf('week'); },
				next: function(value){return new moment(value).add(1,'weeks');},
				label: function(value){return moment(value).format("DD");}
			},"weekday":{
				next: function(value){var next = new Date(value);return new Date(next.setDate(value.getDate()+1));},
				label: function(value){return _weekdays[value.getDay()];}
			},"longday":{
				first: function(value){return moment(value).startOf('day'); },
				next: function(value){return new moment(value).add(1,'days');},
				label: function(value){return moment(value).format("YYYY-MM-DD");}
			},"shortday":{
				first: function(value){return moment(value).startOf('day'); },
				next: function(value){return new moment(value).add(1,'days');},
				...
				first: function(value){var first = new Date(value);return new Date(first.setHours(0,0,0,0)); },
				next: function(value){var next = new Date(value);return new Date(next.setHours(value.getHours()+6));},
				label: function(value){return value.getHours()+":00";}
			},"hour1":{
				first: function(value){var first = new Date(value);return new Date(first.setHours(0,0,0,0)); },
				next: function(value){var next = new Date(value);return new Date(next.setHours(value.getHours()+1));},
				label: function(value){return value.getHours()+":00";}
			},"minutes30":{
				first: function(value){var first = new Date(value);return new Date(first.setMinutes(0,0,0)); },
				next: function(value){var next = new Date(value);return new Date(next.setMinutes(value.getMinutes()+30));},
				label: function(value){return (value.getMinutes()>0)?value.getMinutes():"";}
			},"minutes10":{
				first: function(value){var first = new Date(value);return new Date(first.setMinutes(0,0,0)); },
				next: function(value){var next = new Date(value);return new Date(next.setMinutes(value.getMinutes()+10));},
				label: function(value){return value.getMinutes();}
			}
		},
		base_formatter:{											//rox: 2 base time formatter
			"day":function(value){
				if(!value)return value;
				var v=new Date(value);
				var month = v.getMonth() + 1,day = v.getDate(),hour=v.getHours(),minute=v.getMinutes();
			...
			"hour":function(value){
				...
				hour=hour < 10 ? "0" + hour : hour;
				minute=minute < 10 ? "0" + minute : minute;
				return v.getFullYear() + "-" + month + "-" + day+" "+hour+":00";
			},
			"minute":function(value){
				...
				hour=hour < 10 ? "0" + hour : hour;
				minute=minute < 10 ? "0" + minute : minute;
				return v.getFullYear() + "-" + month + "-" + day+" "+hour+":"+minute;
			}
		},
			
		_create: function(){
			$.ui.rangeSlider.prototype._create.apply(this);
			this._setWheelModeOption(this.options.wheelMode);		//rox
			this.element.addClass("ui-dateRangeSlider");
		},
		_setWheelModeOption: function(value){						//rox
			if (value === null || value === false || value === "scale" ){
				if (this.options.wheelMode !== value){
					this.element.parent().unbind("mousewheel.bar");
				}
				this._bindMouseWheel(value);
		...
		_bindMouseWheel: function(mode){							//rox
			if (mode === "scale"){
				this.element.parent().bind("mousewheel.bar", $.proxy(this._mouseWheelScale, this));
			}
		},
		_mouseWheelScale: function(event, delta, deltaX, deltaY){	//rox
			var ratio=((event.pageX)-(this.container.offset().left))/(this.container.width());
			var omin=this.options.bounds.min.valueOf();
			var omax=this.options.bounds.max.valueOf();
			var wspeed=this.options.wheelSpeed;
			var ointerval=(omax-omin)*2*deltaY*wspeed/100;
			
			var nmin=parseInt(omin- (ointerval*ratio));
			var nmax=parseInt(omax-(ointerval*(ratio-1)));
			this.bounds(nmin,nmax);
			return false;
		},
		...
		_setRulerParameters: function(){		//rox: change scales automatically by (values.max-value.min)
			var rulers={
				"full":{
					"scales":[this.base_scale.year,this.base_scale.shortmonth],
					"step":{months:1},
					"formatter":this.base_formatter["day"]
				},
				"year":{
					"scales":[this.base_scale.longmonth,this.base_scale.shortweek],
					"step":{days:1},
					"formatter":this.base_formatter["day"]
				},
				"month":{
					"scales":[this.base_scale.longweek,this.base_scale.shortday],
					"step":{hours:8},
					"formatter":this.base_formatter["day"]
				},
				"week":{
					"scales":[this.base_scale.longday,this.base_scale.hour6],
					"step":{hours:1},
					"formatter":this.base_formatter["hour"]
				},
				"day":{
					"scales":[this.base_scale.hour1,this.base_scale.minutes30],
					"step":{minutes:5},
					"formatter":this.base_formatter["minute"]
				},
				"hour":{
					"scales":[this.base_scale.hour1,this.base_scale.minutes10],
					"step":{seconds:1},
					"formatter":this.base_formatter["minute"]
				},
			}
			...
			var ntype="";
			var ointerval=this.options.bounds.max.valueOf()-this.options.bounds.min.valueOf();

			if(ointerval>31622400000){
				ntype="full";
			}else if(ointerval>5184000000){
				ntype="year";
			}else if(ointerval>604800000){
				ntype="month";
			}else if(ointerval>86400000){
				ntype="week";
			}else if(ointerval>21600000){
				ntype="day";
			}else{
				ntype="hour";
			}
			this._setFormatterOption("formatter",rulers[ntype]["formatter"]);
			this._setStepOption("step",rulers[ntype]["step"]);
			this.ruler.ruler({
				min: new Date(this.options.bounds.min.valueOf()),
				max: new Date(this.options.bounds.max.valueOf()),
				//scales: this.options.scales,
				scales: rulers[ntype]["scales"]
			});
		},
